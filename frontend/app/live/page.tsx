...
export default function LivePage() {
  ...
  const [message, setMessage] = useState("Enter the backend operator token to inspect live state.");
  ...
  return <main className="mx-auto max-w-7xl p-4 sm:p-6">
    <header className="rounded-2xl border-2 border-rose-500 bg-rose-500/10 p-5"><p className="text-xs font-bold uppercase tracking-[.3em] text-rose-300">Live Trading</p><h1 className="mt-1 text-3xl font-black text-rose-200">LIVE MODE — REAL MONEY</h1><p className="mt-2 text-sm text-slate-300">Default Stage 0 is paper-only. Backend gates, not this page, are authoritative.</p></header>
    <section className="mt-4 grid gap-3 md:grid-cols-2"><Card><label className="text-xs uppercase text-slate-500">Operator token</label><input type="password" value={operatorToken} onChange={e => setOperatorToken(e.target.value)} className="mt-2 w-full rounded border border-slate-700 bg-slate-950 p-2"/><button onClick={() => load().catch(e => setMessage(e.message))} className="mt-3 rounded bg-slate-200 px-4 py-2 text-sm font-bold text-slate-950">Load protected state</button></Card><Card className="border-rose-500/40"><label className="text-xs uppercase text-rose-300">Emergency-role token</label><input type="password" value={emergencyToken} onChange={e => setEmergencyToken(e.target.value)} className="mt-2 w-full rounded border border-rose-700 bg-slate-950 p-2"/><button onClick={stop} className="mt-3 rounded bg-rose-600 px-4 py-2 text-sm font-black">STOP NEW TRADES</button></Card></section>
    <p className="mt-4 rounded-lg border border-slate-700 p-3 text-sm text-slate-300">{message}</p>
    <section className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4"><Card><small>Runtime</small><b className="mt-2 block uppercase">{status.runtime_state ?? "locked"}</b></Card><Card><small>Stage</small><b className="mt-2 block">{status.stage ?? 0} · {status.stage_name ?? "PAPER_ONLY"}</b></Card><Card><small>Emergency stop</small><b className="mt-2 block uppercase text-rose-300">{status.emergency_stop ?? "unknown"}</b></Card><Card><small>Auto execution</small><b className="mt-2 block">{status.auto_execution ? "ON" : "OFF"}</b></Card><Card><small>Account equity</small><b className="mt-2 block">{money(account.equity)}</b></Card><Card><small>Available</small><b className="mt-2 block">{money(account.available_balance)}</b></Card><Card><small>Open positions</small><b className="mt-2 block">{status.open_positions ?? 0}</b></Card><Card><small>Unprotected</small><b className="mt-2 block text-rose-300">{status.unprotected_positions ?? 0}</b></Card></section>
    ...
    <section className="mt-5 grid gap-4 lg:grid-cols-2"><Card><h2 className="font-semibold">Live positions</h2>{positions.length ? positions.map(p => <div key={p.position_id} className="mt-3 border-t border-slate-800 pt-3"><b>{p.pair} · {p.direction}</b><p className={p.protection_status === "protected" ? "text-emerald-300" : "font-bold text-rose-300"}>{p.protection_status === "protected" ? "PROTECTED" : "UNPROTECTED LIVE POSITION"}</p><p className="text-sm text-slate-400">Qty {money(p.quantity)} · entry {money(p.average_price)} · mark {money(p.mark_price)} · leverage {money(p.leverage)}x</p></div>) : <p className="mt-3 text-sm text-slate-500">No reconciled live positions.</p>}</Card><Card><h2 className="font-semibold">Execution activity</h2>{orders.length ? orders.map(o => <div key={o.order_id} className="mt-3 border-t border-slate-800 pt-3 text-sm"><b>{o.pair} · {o.status}</b><p className="text-slate-400">Filled {money(o.filled_quantity)} / {money(o.requested_quantity)}</p></div>) : <p className="mt-3 text-sm text-slate-500">No live execution records.</p>}</Card></section>
  </main>;
}
